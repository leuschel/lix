...
read_preferences :-  read(Term), Term \= end_of_file, !,
   ((Term = preference(Pref,NVal))
      -> (retract(preference(Pref,_)) -> assert(preference(Pref,NVal))
                          ;   add_error(load_preferences,'Unknown preference: ',Pref))
      ;  ((Term = ':-'(dynamic(_))) -> true ; add_error(load_preferences,'Unrecognised:',Term))
    ),
    read_preferences.
read_preferences :- retractall(change_flag).
...
print_preferences :- print_dynamic_pred(preferences,preference,2).
...
preference_default_value(font_size,'Courier 12').
